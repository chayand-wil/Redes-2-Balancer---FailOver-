...
check_link() {
    ...
    if [ $STATUS -eq 0 ]; then
	   ...
	   if ! ip route show default | grep -q "${GW2}"; then
		...
		ip route del default via ${GW1} dev ${IFACE1} 2>/dev/null || true
                ip route del default via ${GW2} dev ${IFACE2} 2>/dev/null || true
                #AGREGAmos la ruta correcta
		...
		#eliminar todas las rutas por defecto 
